...
package org.eclipse.ui.examples.views.properties.tabbed.resus.properties.IOProvider;
...
import de.clausthal.tu.ielf.resusdesigner.model.commands.IOProvider.SetIOProviderIdCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.IOProvider.SetIOProviderIndexCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.IOProvider.SetIOProviderInputFilesCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.IOProvider.SetIOProviderLogFileCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.IOProvider.SetIOProviderNumberOfInputPinsCommand;
 ...
 */
public class IOProviderConfigSection extends AbstractSection {
	 ...
	 /**
		...
		private TextChangeHelper IdChangelistener = new TextChangeHelper() {
			...
			public void textChanged(Control control) {
				...
				SetIOProviderIdCommand rnc=new SetIOProviderIdCommand();
				rnc.setPart((IOProvider)getElement());				
				rnc.setId(id);			
	...
	private TextChangeHelper NumberOfInputPinsFieldChangelistener = new TextChangeHelper() {
		...
		public void textChanged(Control control) {
			...
			d.height=((IOProvider)getElement()).getSize().height;
			d.width=((IOProvider)getElement()).getSize().width;
			if(num>4) 
				d.height=(25*num)+25;
			
				
			
			SetIOProviderNumberOfInputPinsCommand rnc=new SetIOProviderNumberOfInputPinsCommand();
			rnc.setPart((IOProvider)getElement());
	...
	private void ChangeLogFileText(){
		SetIOProviderLogFileCommand rnc=new SetIOProviderLogFileCommand();
		rnc.setPart((IOProvider)getElement());			
	...
	private void ChangeIndexText(){
		SetIOProviderIndexCommand rnc=new SetIOProviderIndexCommand();
		rnc.setPart((IOProvider)getElement());			
	...
	private void ChangeInputFilesList(){
		SetIOProviderInputFilesCommand rnc=new SetIOProviderInputFilesCommand();
		rnc.setPart((IOProvider)getElement());			
					...
					public void widgetSelected(SelectionEvent e) 
						{
							...
							buttons.get(filename).dispose();
							buttons.remove(filename);
							
							editButtons.get(filename).dispose();
							editButtons.remove(filename);
...
//							viewer.refresh();
                ...
                else
                {
                    ...
                    button.addSelectionListener(new SelectionAdapter() 
                    {
						...
						public void widgetSelected(SelectionEvent ex) 
						{
							...
							String[] btns={};
							SampleViewer sv=new SampleViewer(composite.getShell(), "Sample", null, "", 0, btns, 0, filename);
							sv.create();
							sv.open();
							//-----------
						}
		...
		lblHelpTip.setText(ResusMessages.IOProvider_Configuration_information);
			...
			if(!filenames.contains(filePath))
				{
					...
					//((IOProvider)getElement()).setFileNames((ArrayList<String>)filenames);
					ChangeInputFilesList();
	...
	public void refresh() {
		...
		if(!(getElement() instanceof IOProvider)){
			MessageDialog.openError(composite.getShell(), "Error","Element is not an IOProvider Instance");
			return;
		}
		...
		try {
			...
			String id = (String) ((IOProvider) getElement()).getId();
			
			txtId.setText(id);
			
			txtIndex.setText((String) ((IOProvider) getElement()).getIndex());
			txtNumberOfInputPins.setText(String.valueOf(((IOProvider)getElement()).getNumberOfInputs()));
			txtLogFileName.setText(((IOProvider)getElement()).getLogFileName());
			
			if(((IOProvider) getElement()).getFileNames()!=null)
			filenames=((IOProvider) getElement()).getFileNames();
