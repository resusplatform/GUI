...
import de.clausthal.tu.ielf.resusdesigner.figures.IOProviderFeedbackFigure;
...
import de.clausthal.tu.ielf.resusdesigner.figures.LogicFlowFeedbackFigure;
...
import de.clausthal.tu.ielf.resusdesigner.model.commands.AddCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.ChangeGuideCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.CloneCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.CreateCommand;
import de.clausthal.tu.ielf.resusdesigner.model.commands.SetConstraintCommand;

public class LogicFixedXYLayoutEditPolicy extends
		org.eclipse.gef.editpolicies.XYLayoutEditPolicy {

	public LogicFixedXYLayoutEditPolicy(XYLayout layout) {
		super();
		setXyLayout(layout);
	}

	protected Command chainGuideAttachmentCommand(Request request,
			ResusSubpart part, Command cmd, boolean horizontal) {
		Command result = cmd;
	...
	protected Command createAddCommand(ChangeBoundsRequest request,
		...
		ResusSubpart part = (ResusSubpart) child.getModel();
		Rectangle rect = (Rectangle) constraint;

		AddCommand add = new AddCommand();
		add.setParent((ResusDiagram) getHost().getModel());
		add.setChild(part);
		add.setLabel(ResusMessages.LogicXYLayoutEditPolicy_AddCommandLabelText);
		...
		setConstraint
				.setLabel(ResusMessages.LogicXYLayoutEditPolicy_AddCommandLabelText);
	...
	protected IFigure createSizeOnDropFeedback(CreateRequest createRequest) {
		...
		if (createRequest.getNewObject() instanceof ResusModel)
			figure = new ResusModelFeedbackFigure((ResusModel)createRequest.getNewObject());
		if (createRequest.getNewObject() instanceof IOProvider)
			figure = new IOProviderFeedbackFigure((IOProvider)createRequest.getNewObject());
		if (createRequest.getNewObject() instanceof ResultConverter)
			figure = new ResultConverterFeedbackFigure((ResultConverter)createRequest.getNewObject());		
	...
	protected Command getCloneCommand(ChangeBoundsRequest request) {
		...
		clone.setParent((ResusDiagram) getHost().getModel());
		...
		while (i.hasNext()) {
			...
			clone.addPart((ResusSubpart) currPart.getModel(),
	...
	protected Command getCreateCommand(CreateRequest request) {
		...
		create.setParent((ResusDiagram) getHost().getModel());
		ResusSubpart newPart = (ResusSubpart) request.getNewObject();
		create.setChild(newPart);
		Rectangle constraint = (Rectangle) getConstraintFor(request);
		create.setLocation(constraint);
		create.setLabel(ResusMessages.LogicXYLayoutEditPolicy_CreateCommandLabelText);
	...
	protected Insets getCreationFeedbackOffset(CreateRequest request) {
		if ( request.getNewObject() instanceof ResusModel)
